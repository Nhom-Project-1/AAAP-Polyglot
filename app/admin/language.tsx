...
const mockData: Language[] = Array.from({ length: 100 }).map((_, i) => ({
ma_ngon_ngu: `NG${i + 1}`,
ten_ngon_ngu: `Ngôn ngữ ${i + 1}`,
mo_ta: `Mô tả cho ngôn ngữ ${i + 1}`,
}))
...
export default function AdminLanguage() {
  ...
  const [data, setData] = useState<Language[]>(mockData)
  ...
  const filteredData = data.filter(lang =>
    lang.ten_ngon_ngu.toLowerCase().includes(searchTerm.toLowerCase()) ||
    lang.mo_ta.toLowerCase().includes(searchTerm.toLowerCase())
  )
  ...
  const handleSave = (lang: Language, isEdit: boolean) => {
    const newErrors = { ten_ngon_ngu: "", mo_ta: "" }
    if (!lang.ten_ngon_ngu.trim()) {
      newErrors.ten_ngon_ngu = "Tên ngôn ngữ không được để trống"
      setErrors(newErrors)
  }
    if (!lang.mo_ta.trim()) {
    ...
    if (isEdit) {
      toast.dismiss()
      setData(prev => prev.map(l => l.ma_ngon_ngu === editingLang?.ma_ngon_ngu ? { ...l, ...lang } : l))
      toast.success("Sửa ngôn ngữ thành công!")
    } else {
      toast.dismiss()
      const maxId = data.reduce((max, item) => {
        const num = parseInt(item.ma_ngon_ngu.replace('NG','')) || 0
        return num > max ? num : max
      }, 0)
      lang.ma_ngon_ngu = `NG${maxId + 1}`
                ...
                <>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex flex-col">
                      ...
                      <input id="name" type="text" defaultValue={editingLang?.ten_ngon_ngu || ""} className="border px-3 py-2 rounded-md w-full" onChange={() => setIsDirty(true)} />
                    ...
                    <div className="flex flex-col">
                      ...
                      <input id="desc" type="text" defaultValue={editingLang?.mo_ta || ""} className="border px-3 py-2 rounded-md w-full" onChange={() => setIsDirty(true)} />
              ...
              {modalType === "delete" && langToDelete && (
                <>
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      onClick={() => {
                        setData(prev => prev.filter(l => l.ma_ngon_ngu !== langToDelete.ma_ngon_ngu))
                        setModalType(null)
