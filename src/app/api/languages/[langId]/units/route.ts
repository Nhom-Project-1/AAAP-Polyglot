...
export async function GET(req: NextRequest, ctx: { params: Promise<Params> }) {
  try {
    ...
    const [units, allLessons, completedLessons] = await Promise.all([
      // Query 1: Fetch units
      db
        .select()
        .from(s.unit)
      ...
      // Query 2: Fetch all lessons for this language (we'll filter by unit later)
      db
        .select()
        .from(s.bai_hoc)
        .innerJoin(s.unit, eq(s.bai_hoc.ma_don_vi, s.unit.ma_don_vi))
        .where(eq(s.unit.ma_ngon_ngu, langId))
        .orderBy(s.bai_hoc.ma_bai_hoc)
        .then(rows => rows.map(r => r.bai_hoc)),
      
      // Query 3: Fetch completed lessons (only if user is logged in)
      maNguoiDung
        ? db
    ...
    const lessons = allLessons.filter(l => unitIds.includes(l.ma_don_vi as number));

    // Build completed lessons set
    const completedLessonIds = new Set<number>();
    completedLessons.forEach(c => completedLessonIds.add(c.ma_bai_hoc));
